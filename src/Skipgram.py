...
class Skipgram(object):
    ...
    def __call__(self, X):
        ...
        h = torch.matmul(self.weights[0].T, X)
    ...
    def backward(self, X, t):
        ...
        dw = torch.clip(torch.matmul(
            self.track_state['h'][-1], ei.T), min=-self.clip, max=self.clip)
        self.weights[1] = self.weights[1] - \
            learning_rate * dw  # weight update rule
        # input layer -> hidden layer
        eh = torch.matmul(self.weights[1], ei)
        dw = torch.clip(torch.matmul(X, eh.T), min=-self.clip, max=self.clip)
